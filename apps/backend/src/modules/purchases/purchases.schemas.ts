...
export const createPurchaseBillSchema = z.object({
  ...
  billDate: dateSchema,
  deliveryNoteNumber: z.string().trim().max(80).optional().or(z.literal("")).transform((value) => value || null),
  buyerOrderNumber: z.string().trim().max(80).optional().or(z.literal("")).transform((value) => value || null),
  buyerOrderDate: dateSchema.optional().or(z.literal("")).or(z.null()).transform((value) => value || null),
  dispatchDocumentNumber: z.string().trim().max(80).optional().or(z.literal("")).transform((value) => value || null),
  deliveryNoteDate: dateSchema.optional().or(z.literal("")).or(z.null()).transform((value) => value || null),
  dispatchedThrough: z.string().trim().max(120).optional().or(z.literal("")).transform((value) => value || null),
  destination: z.string().trim().max(180).optional().or(z.literal("")).transform((value) => value || null),
  termsOfDelivery: optionalTextSchema,
