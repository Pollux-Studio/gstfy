...
import {
  clearSalesInvoicePdfCache,
  downloadBlob,
  fetchSalesInvoicePdf,
} from "@/lib/sales/sales-invoice-client"

export function SalesInvoiceViewerPage({ invoiceId }: { invoiceId: string }) {
  const accessToken = getStoredAuthSession()?.session.accessToken ?? ""
  const forceNextFetchRef = React.useRef(false)
  const invoiceQuery = useQuery({
    queryKey: ["sales-invoice-pdf", invoiceId],
    queryFn: () => {
      const force = forceNextFetchRef.current
      forceNextFetchRef.current = false

      return fetchSalesInvoicePdf(accessToken, invoiceId, { force })
    },
    enabled: accessToken.length > 0 && invoiceId.length > 0,
    gcTime: 60 * 1000,
    staleTime: 0,
  })
        ...
        <div className="flex min-w-0 flex-1 flex-wrap items-center justify-end gap-2">
          <Button
            ...
            onClick={() => {
              ...
              clearSalesInvoicePdfCache(accessToken, invoiceId)
            ...
            <div className="flex h-full items-center justify-center p-6">
              <Empty>
                <EmptyHeader>
                  ...
                  <EmptyTitle>Sales invoice PDF could not load</EmptyTitle>
                  <EmptyDescription>
                    The document renderer could not fetch this sales bill. Refresh and try again.
                  </EmptyDescription>
                </EmptyHeader>
                <EmptyContent>
                  <Button
