...
} from "@/lib/sales/sales-invoice-pdf"
...
export async function GET(
  ...
  const backendResponsePromise = fetch(
    `${API_BASE_URL}${API_BASE_PATH}/sales/invoices/${encodeURIComponent(invoiceId)}`,
    {
      cache: "no-store",
  ...
  const renderSettingsPromise = getSalesInvoiceRenderSettings(backendHeaders).then(
    (settings) => ({
  ...
  const payload = (await backendResponse.json()) as { invoice: SalesInvoiceDetail }
  const renderStartedAt = performance.now()
  const pdf = await renderSalesInvoicePdf(payload.invoice, {
    seller: renderSettings.seller,
    templateCode: renderSettings.salesInvoiceTemplate,
  ...
  return new NextResponse(pdf, {
    headers: {
      ...
      "Content-Disposition": `inline; filename="${safeFileName(payload.invoice.invoiceNumber)}.pdf"`,
...
async function getSalesInvoiceRenderSettings(headers: Headers): Promise<{
  ...
  return {
    ...
    salesInvoiceTemplate:
      payload.invoiceSettings?.salesInvoiceTemplate ??
...
function safeFileName(value: string) {
  ...
  return sanitized ? `sales-${sanitized}` : "sales-invoice"
}
