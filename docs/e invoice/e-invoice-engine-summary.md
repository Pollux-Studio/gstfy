...
The current implementation uses a mock provider adapter only.
...
This keeps the domain and UI ready while avoiding dependency on live IRP/GSP credentials.
...
- No live IRP/GSP integration yet.
- No NIC/GSTN production credential flow yet.
...
The E-Invoice Engine is structurally ready as a mock-provider-backed compliance workflow:
...
Before production IRN submission, replace the mock adapter with the selected GSP/IRP provider adapter and add credential/configuration management per business GST registration.
